...
(function($) {
    $.fn.emoji = function(options) {
        var defaults = {
        	content_el:"#content",
            list: [{name:"QQ表情",code:"qq_",path:"./face/emoji1/",suffix:".gif",max_number:75},{name:"emoji表情",code:"em_",path:"./face/emoji2/",suffix:".png",max_number:52}]
        };
		...
		$(this).click(function(e){
			if($(".emoji-box").length <=0){
				...
				console.log("sh",$(document).scrollTop());
				console.log("posision",$(this).position());
				console.log("th",$(".emoji-box").height());
				var dh = $(document).height();
				$("ul.emoji-ul").hide();
				$(".emoji-box>.emoji-btn-box>span").first().css("color","blue")
				$("#"+list[0].code+"emoji").show();
				var offset = $(this).position();
				if(offset.top+300 >dh){//在底部的时候
					$(".emoji-box").css("top",offset.top-220-$(this).height());
					$(".emoji-box").css("left",offset.left);
					$(this).parent().find("em.tip").css("left",-1000)
					$(this).parent().find("em.tip2").css("left",-1000)
				}else{
