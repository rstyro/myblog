...
(function($){
	...
	$.fn.initComment=function(options){
		...
		if(typeof(arr) != "undefined" && arr.length>0){
			for(var i=0;i<arr.length;i++){
				floorNum = addComment($(this),arr[i],direction,floorNum,userName,reply_method,praise_method);
			}
		}
		return floorNum;
	}
	...
	$.fn.comment=function(options){
		...
		if(callback && callback.status == "success"){
			floorNum = addComment($(this),callback.data,direction,floorNum,userName,reply_method,praise_method);
		}
		return floorNum;
	}
	...
	function pariseAction(el,praise_method){
		...
		commentBody.comment_id = el.parent().parent().attr("comment_id");
	...
	//添加 评论
	function addComment(el,obj,direction,floorNum,userName,reply_method,praise_method){
	...
	function textareaBoxListen(parentEL,obj,reply_method){
		...
		parentEL.find(".smile-reply").emoji({content_el:parentEL.find(".child-comment-list>.commentbox>textarea"),list: [{name:"QQ表情",code:"qq_",path:_ctx+"/comment/face/emoji1/",suffix:".gif",max_number:75},{name:"emoji表情",code:"em_",path:_ctx+"/comment/face/emoji2/",suffix:".png",max_number:52}]});
	...
	function createCommentListBox(obj,floorNum){
		...
		*/
		obj.content = replace_em(obj.content);
		var commentstr = "<div class='comment-list-box' comment_id='"+obj.comment_id+"'>";
			...
			commentstr = commentstr + "<p class='content'>"+obj.content+"</p>";
	...
	function createReplyHtml(obj){
		...
		var replyStr = "<div class='child-comment'>";
			replyStr = replyStr+"<p><a href='javascript:void(0)'>"+obj.userName+"</a>:<a href='javascript:void(0)'>@"+obj.replyUserName+"</a>"+obj.content+"</p>";
	...
	function replace_em(str){
		...
		//str = str.replace(/\[qq_([0-9]*)\]/g,"<img src='/comment/face/emoji1/$1.gif' />");
		//str = str.replace(/\[em_([0-9]*)\]/g,"<img src='/comment/face/emoji2/$1.png'  />");
		//str = str.replace(/\[other_([0-9]*)\]/g,"<img src='/comment/face/emoji3/$1.jpg' />");
		return str;
